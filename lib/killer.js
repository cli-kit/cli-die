 ...
 */
function match(doc, ptns, cb) {
  var result = {}, k, list, item, pids = [];

  //console.log('match with pids %j', doc._pids);
    ...
    for(i = 0;i < list.length;i++) {
      for(j = 0;j < ptns.length;j++) {
        ...
        if(ptns[j].test('' + list[i])) {
          // TODO :return pid
          //return list[i];
          matches.push({match: list[i], index: i});
        }
      }
    }
  ...
  for(k in doc) {
    //console.log('testing %s', k);
    list = doc[k];
    if(item = find(ptns, list)) {
      pids = pids.concat(item);
    }
  }
  cb(null, result);
}
